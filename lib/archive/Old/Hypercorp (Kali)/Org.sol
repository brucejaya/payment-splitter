...
import '../../interfaces/IOrgextension.sol';
...
contract Org is Orgtoken, Multicall, NFThelper, ReentrancyGuard {
    ...
    function init(
        ...
        Orgtoken._init(name_, symbol_, paused_, voters_, shares_);
    ...
    function _vote(
        ...
        // which is checked for overflow in `Orgtoken` contract
        unchecked { 
            if (approve) {
            ...
            unchecked {
                ...
                if (prop.proposalType == ProposalType.EXTENSION) 
                    for (uint256 i; i < prop.accounts.length; i++) {
                        ...
                        if (prop.payloads[i].length != 0) IOrgextension(prop.accounts[i])
    ...
    function _countVotes(
        ...
        if (voteType == VoteType.SIMPLE_MAJORITY_QUORUM_REQUIRED || voteType == VoteType.SUPERMAJORITY_QUORUM_REQUIRED) {
            ...
            // supply are checked in `Orgtoken` contract
            unchecked {
    ...
    function callExtension(
        ...
        if (extensions[msg.sender]) {
            ...
            account = msg.sender;
            (mint, amountOut) = IOrgextension(extension).callExtension{value: msg.value}
